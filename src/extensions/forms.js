...
const number_of_1_16_100_items = 5;
const sizes = new Map([
    ...
    this.#buttonArray.splice(slot, 1, [
      `stack#${Math.min(Math.max(stackSize, 1) || 1, 99)
        ...
        .padStart(
          2,
          "0",
        )}§r${itemName ?? ""}§r${itemDesc?.length ? `\n§r${itemDesc.join("\n§r")}` : ""}`,
      (ID + (ID < 262 ? 0 : number_of_1_16_100_items)) * 65536 +
        !!enchanted * 32768 || texture,
    ]);
    return this;
  }
  pattern(pattern, key) {
    for (let i = 0; i < pattern.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (key[letter]) {
          ...
          const ID =
            typeIdToDataId.get(data.texture) ?? typeIdToID.get(data.texture);
          this.#buttonArray.splice(slot, 1, [
            ...
            (ID + (ID < 262 ? 0 : number_of_1_16_100_items)) * 65536 +
              !!data?.enchanted * 32768 || data.texture,
          ]);
        }
      }
    }
